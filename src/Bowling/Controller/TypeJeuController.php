...
use CHRIST\Modules\Bowling\Form\TypeJeuForm;
 ...
 */
class TypeJeuController extends GlobalController {
    ...
    protected function ajoutAction(Request $request, Application $app) {
        return $this->getForm(new TypeJeu(), 'src/Bowling/Resources/views/type/ajout.html.twig');
    }

    protected function updateAction(Request $request, Application $app) {
        $record = $this->getCurrent();

        return $this->getForm($record, 'src/Bowling/Resources/views/type/ajout.html.twig');
    }
    ...
    protected function deleteAction(Request $request, Application $app) {
        ...
        if ($record->getEvenements()->count() == 0) {
            ...
            $this->app['session']->getFlashBag()->add('info', 'Le type "' . $record->getTypeJeu() . ' - ' . $record->getLibelle() . '" a bien été supprimée');
        } else {
     ...
     * Return form view
     */
    private function getForm(TypeJeu $entity, $view) {
        $form = $this->app["form.factory"]->create(
                new TypeJeuForm(), $entity, array(
