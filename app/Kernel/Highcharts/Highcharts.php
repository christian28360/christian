 ...
 * Interface PHP avec la librairie Highcharts
 * @source = http://www.code2learn.com/2012/04/using-highcharts-with-php-mysql-and.html
        ...
        foreach ($options as $opt_key => $opt_name) {
            if (is_string($opt_key)) {
                if (is_object($opt_name)) {
                    $root[$opt_key] = array();
                    $root[$opt_key] = $this->set_local_options($opt_name, $root[$opt_key]);
                } else
