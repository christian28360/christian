...
/*
 * Allow host parameter
 */

namespace CHRIST\Common\Kernel\Providers;
    ...
    public function register(Application $app)
    {
        ...
        $app['security.access_map'] = $app->share(function ($app) {
            ...
            foreach ($app['security.access_rules'] as $rule) {
                ...
                $matcher   = array('path' => null, 'host' => null);
                
                
                if (isset($rule['path']) && !empty($rule['path'])) {
                    $matcher['path'] = $rule['path'];
                }
                
                if (isset($rule['host']) && !empty($rule['host'])) {
                    $matcher['host'] = $rule['host'];
                }
                
                $attributes = array_diff($rule, $matcher);
                
                $matcher = new RequestMatcher($matcher['path'], $matcher['host']);

                $map->add($matcher, (array) $attributes, null);
            }
